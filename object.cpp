...
void Object::setMaterial(GLfloat red, GLfloat green, GLfloat blue, GLfloat alpha, GLfloat specular, GLfloat shininess,
                         GLfloat emission) {
    color[0] = red;
    color[1] = green;
    color[2] = blue;
...
void Object::setNormalmode(int normalmode) {
    normal_mode = normalmode;
}


bool Object::readClusterVertex(char **r_line, int *p, int *t, int *n) {
    ...
    while (h[0] != 0 && h[0] == ' ') h++;                            // �berlese f�hrende Blanks

    if (sscanf(h, "%i", &p[0]) ==
        1)                            // Vertex-Index wurde gelesen, es folgt ein Slash oder ein Blank bzw. das Zeilenende
    {
        ...
        switch (h[0]) {
            ...
            case ' ':                                            // Leerzeichen, d.h. kein Vertex-Cluster
                ...
                return true;
            case '/':                                            // Vertex-Cluster
                h++;                                            // den Slash �berlesen, es folgt  die Texturkoordinate oder ein zweiter Slash

                sscanf(h, "%i", &t[0]);                            // wir versuchen die Texturkoordinate zu lesen
                while (h[0] != 0 && h[0] != '/' && h[0] != ' ') h++;    // zum zweiten Slash vorsetzen

                if (h[0] ==
                    '/')                                // die Vertexnormale muss eigentlich folgen, wenn sie fehlt setzen wir die Standardnormale
                {
                    h++;                                        // den zweiten Slash �berlesen
                    if (h[0] == ' ')                            // Fehler: Normale fehlt
                    {
                        n[0] = 0;
                    } else {
                        sscanf(h, "%i", &n[0]);
                        while (h[0] != 0 && h[0] != ' ') h++;        // �ber die Vertexnormale vorsetzen
                    }
                    r_line[0] = h;
                    return true;
                }
...
void Object::load(const char *filename, bool use_vbos) {
    if (!filename) return;
...
#define ZEILENLAENGE 200
    ...
    if (npoints > 0 && ntris > 0) {
        ...
        tris = (GLint *) malloc(
            ntris * 3 * sizeof(GLint));                                    // Index-Array f�r Fl�chen anlegen

        if (cltexcoords) {
            //use_vbos = false;
            clustertexcoords = true;
            i_texcoords = (GLint *) malloc(
                3 * ntris * sizeof(GLint));                    // importierte TexturkoordinatenpIndexe
            texcoords = (GLfloat *) malloc(
                (ntexcoords + 1) * 2 * sizeof(GLfloat));        // u,v - Koordinaten aus dem File
            ...
            switch (s[0]) {
                case 'v':
                    switch (s[1]) {
                        case ' ': {
                            GLfloat *p = &points[3 *
                                                 npoints];                                    // pointer auf Vertex setzen
                            if (sscanf(&s[2], "%f %f %f", &p[0], &p[1], &p[2]) ==
                                3)                    // 3 Floats lesen
                                npoints++;
                            else {
                                printf("Fehler in Wavefrontdatei %s in Zeile %d\n", filename, line);
                        ...
                        case 'n': {
                            ...
                            if (sscanf(&s[2], "%f %f %f", &n[0], &n[1], &n[2]) ==
                                3)                    // 3 Floats lesen
                                hnormals++;
                            else {
                                printf("Fehler in Wavefrontdatei %s in Zeile %d\n", filename, line);
                ...
                case 'f':
                    ...
                    if (readClusterVertex(&h, &p[0], &t[0], &n[0]) &&
                        ...
                        for (int i = 0; i < 3; i++)  // Textur- und Normalenindizes nur dekrementieren, wenn gelesen
                        {
                            if (cltexcoords) {
                                if (t[i] > 0) {
                                    if (t[i] <= ntexcoords) // Index liegt im g�ltigen Bereich
                                        ot[i] = t[i];        // sonst steht da (-1)
                                    else {
                                        //printf("Index zu gro�, korrigiert\n\r");
                                        ot[i] = 0;
                                    }
                                } else
                        ...
                        {
                            int *q = &tris[3 * ntris];                                    // das n�chste Dreieck
                            q[0] = p[0];
                            q[1] = p[2];
                            q[2] = hp - 1;
                            p = q;
...
#pragma region NORMALEN_BERECHNEN
    ...
    if (!clusternormals) {    // wir m�ssen interpolierte Normalen berechnen
...
#pragma region VBO_Erzeugen
    ...
    //////////////////////////////////////////////////
    if (use_vbos) {
        // wir m�ssen die Daten f�r Drawcalls aufbereiten, weil ein Vertex mit mehreren Normalen und TCs vorkommen kann
        // (beim vertex split)
        ...
        int *in = NULL;
        int *it = NULL;
        // �ber alle Punkte der Dreiecke, Normalen und ggf. Texturkoordinaten ins VBO kopieren
        for (int i = 0; i < numtris; ++i) {
            p = &tris[3 * i];
            if (clusternormals)
                in = &i_normals[3 * i];
            ...
            GLfloat *fn = &f_normals[3 * i];            // Zeiger auf die Fl�chennormale
            for (int j = 0; j < 3; j++) {
        ...
        if (vbo) // kein Fehler beim Generieren
        {
            ...
            // Vertexdaten �bertragen (Koordinaten, dann Vertexnormalen, dann Texturkoordinaten)
